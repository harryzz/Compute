...
class Subgraph::NodeCache {
  public:
    ...
    static_assert(sizeof(Type) == (sizeof(void *) == 8 ? 40 : 20)); // [wasm32] 4 raw ptrs 8->4 (=16); no tail pad
    struct Item {
    ...
    static_assert(sizeof(Item) == (sizeof(void *) == 8 ? 32 : 24)); // [wasm32] 2 raw ptrs (next,prev) 8->4
    struct ItemKey {
