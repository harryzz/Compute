...
#if defined(__wasi__)
// [wasm] plain-C-callback variant of attribute_modify (mirror of the above, but `modify` uses the C
// calling convention so the Swift @convention(c) thunk matches it). Core method — needs Node/type
// internals; the C-ABI wrapper lives in IAGWasiClosureShim.cpp. See WasiClosureShim.mutateBody.
