...
#if defined(__wasi__)
    // [wasm port] plain C-CC thunk: Swift passes a @convention(c) trampoline (the swiftcall closure
    // ABI mismatches across the C boundary on wasm). update() invokes it with this signature.
    void (*_update)(void *body, IAGAttribute attribute, void *context);
#else
