...
class indirect_pointer_vector {
  ...
  public:
    ...
    indirect_pointer_vector(const indirect_pointer_vector &) = delete;
    indirect_pointer_vector &operator=(const indirect_pointer_vector &) = delete;

    // Move

    indirect_pointer_vector(indirect_pointer_vector &&other) : _data(std::exchange(other._data, 0)) {};
    indirect_pointer_vector &operator=(indirect_pointer_vector &&other) {
        if (this != &other) {
            if (has_vector()) {
                vector_type *vector_pointer = get_vector();
    ...
    reference operator[](size_type pos) {
        assert(pos < size());
        if (has_vector()) {
            auto &vector_pointer = *get_vector();
